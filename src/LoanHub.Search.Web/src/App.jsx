...
const storedApiBaseKey = "loanhub_api_base";
...
function App() {
  const [apiBase, setApiBase] = useState(
    () => localStorage.getItem(storedApiBaseKey) ?? envApiBase
  );
  ...
  useEffect(() => {
    if (apiBase) {
      localStorage.setItem(storedApiBaseKey, apiBase);
    } else {
      localStorage.removeItem(storedApiBaseKey);
    }
  }, [apiBase]);
  ...
  const callApi = async ({ path, method = "GET", body, withAuth = false }) => {
    ...
    try {
      ...
      throw new Error(
        "Nie udało się połączyć z API. Sprawdź adres bazowy i upewnij się, że backend działa."
      );
    }
